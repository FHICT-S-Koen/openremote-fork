                            ...
                            ${when(this.mapConfig, () => html`
                                <div class="global-settings-container">
                                    <div class="custom-server-group">
                                        <div class="subheader">${i18next.t("configuration.global.mapServer")}</div>
                                        <span>${i18next.t("configuration.global.mapServerDescription")}</span>
                                        <or-mwc-input class="input" outlined
                                            .value="${this.mapConfig.sources?.['external']?.url}" 
                                            .type="${InputType.URL}"
                                            .label="${i18next.t("configuration.global.mapServerPlaceholder")}"
                                            placeholder="https://api.example.com/tileset/{z}/{x}/{y}/{secret}"
                                            @or-mwc-input-changed="${(e: OrInputChangedEvent) => {
                                                    if (e.detail.value) {
                                                        Object.assign(this.mapConfig.sources['external'] ??= {}, { 
                                                            type: 'vector', 
                                                            url: e.detail.value
                                                        })
                                                    } else {
                                        ...
                                        ></or-mwc-input>
                                        <div class="subheader">${i18next.t("configuration.global.apiKey")}</div>
                                        <span>${i18next.t("configuration.global.apiKeyDescription")}</span>
                                        <or-mwc-input class="input" outlined 
                                            .type="${InputType.PASSWORD}"
                                            .label="${i18next.t("configuration.global.apiKey")}"
                                            .value="${this.mapConfig.sources?.['external']?.secret}" 
                                            .disabled="${!this.mapConfig.sources?.['external']?.url?.includes?.('{secret}')}"
                                            @or-mwc-input-changed="${(e: OrInputChangedEvent) => {
                                                    if (e.detail.value) {
                                                        let external = this.mapConfig.sources['external'];
                                                        Object.assign(external, { secret: e.detail.value })
                                                    } else {
